...
<body>
    <div class="container mt-4">
        ...
        <form action="../Add/process_add_importance_branches_subjectvcc.php" method="POST">
            <div class="mb-3">
            ...
            <div class="mb-3">
                <label for="subjectvcc_id" class="form-label">SubjectM6 ID</label>
                <select class="form-subjectVCC_name" id="subjectVCC_name" name="subjectVCC_name" required>
                    <option value="">เลือก Attention ID</option>
                    <?php
                    // Include connection file and fetch branch data from database
                    include "../../Connent/connent.php";
                    $sql = "SELECT subjectVCC_name FROM subjectvcc";
                    $result = $connect->query($sql);
                    while ($row = $result->fetch_assoc()) {
                        echo "<option value='" . $row['subjectVCC_name'] . "'>" . $row['subjectVCC_name'] . "</option>";
                    }
                    ?>
                </select>
            </div>
            ...
            <a href="../Manage/manage_importance_subjectvcc.php" class="btn btn-secondary btn-block">กลับ</a>
        </form>
    </div>
</body>
