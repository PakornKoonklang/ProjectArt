...
if (isset($_GET['subjectM6_id'])) {
    ...
    $stmt = $connect->prepare("DELETE FROM subjectm6 WHERE subjectM6_id = ?");

    // Check if the prepare statement succeeded
    if ($stmt) {
        // Bind parameters and execute the statement
        $stmt->bind_param("i", $subjectM6id); // Use "i" for integer
        ...
        if ($stmt->execute()) {
            ...
            $resequenceSql = "
                SET @i = 0;
                UPDATE subjectm6
                SET subjectM6_id = (@i := @i + 1)
                ORDER BY subjectM6_id;
            ";
