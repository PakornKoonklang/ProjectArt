...
if ($admin_level_id == 10) {
    ...
    $sql = "SELECT b.*, br.branches_Name, s.subjectVC_name
            FROM branches_subjectvc AS b 
            INNER JOIN branches AS br ON b.branches_Id = br.branches_Id
            INNER JOIN subjectvc AS s ON b.subjectVC_Id = s.subjectVC_id";
    $count_sql = "SELECT COUNT(*) AS total FROM branches_subjectvc";
} else {
    ...
    $count_sql = "SELECT COUNT(*) AS total FROM branches_subjectvc WHERE branches_Id = $admin_level_id";
}
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0 text-dark">กำหนดค่าความสำคัญของวิชาในเเต่ละสาขา ในระดับชั้น ปวช.</h1>
                        </div>
                        <div class="col-sm-6">
                            <div class="d-flex justify-content-end">
                                ...
                                <?php if ($admin_level_id == 10) { ?>
                                    <a href='../Add/add_importance_branches_subjectvc.php' class='btn btn-success ml-3'><i class="fas fa-plus"></i> เพิ่ม</a>
                                <?php } ?>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <table class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo htmlspecialchars($row['subjectVC_name']); ?></td>
                                                <td><?php echo htmlspecialchars($row['subject_Multiplier']); ?></td>
                                                <td>
                                                    <div class="btn-group" role="group">
                                                        <?php if ($admin_level_id == 10 || $row['branches_Id'] == $admin_level_id) { ?>
                                                            <a href='../Edit/edit_importance_branches_subjectvc.php?id=<?php echo $row['id']; ?>' class='btn btn-warning mx-1'><i class="fas fa-edit"></i></a>
                                                            <a href='../Delete/process_delete_importance_subjectvc.php?id=<?php echo $row['id']; ?>' class='btn btn-danger' onclick="return confirm('คุณต้องการลบข้อมูลหรือไม่?')"><i class="fas fa-trash"></i></a>
                                                        <?php } ?>
                                                    </div>
                                                </td>
                                            </tr>
