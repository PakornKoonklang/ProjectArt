...
while ($branch = $result_branches->fetch_assoc()) {
    ...
    foreach ($grades as $subjectId => $grade) {
        if (is_numeric($grade)) {
            ...
            $sql_subject_multiplier = "SELECT subject_Multiplier FROM branches_subjectm6 WHERE branches_Id = $branch_id AND subjectM6_Id = $subjectId";
...
<head>
    ...
    <style>
        body {
            ...
            background-size: cover; /* ทำให้รูปพื้นหลังครอบคลุมพื้นที่ทั้งหมด */
            background-position: center; /* จัดตำแหน่งรูปพื้นหลังตรงกลาง */
            background-attachment: fixed; /* ทำให้รูปพื้นหลังไม่เลื่อนตามการเลื่อนของหน้าเว็บ */
        }
        .container {
            background-color: rgba(255, 255, 255, 0.8); /* ทำให้พื้นหลังของ container โปร่งใสเล็กน้อยเพื่อให้เนื้อหาเด่นชัดขึ้น */
            padding: 20px; /* เพิ่มช่องว่างภายใน container */
            border-radius: 10px; /* ทำให้มุมของ container โค้งมน */
        }
    </style>
</head>
<body>
